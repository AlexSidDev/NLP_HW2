...
class Trainer:
    def __init__(self, model,
        ...
        self.criterion = nn.CrossEntropyLoss(ignore_index=-100) if not use_dice else DiceLoss()
    ...
    def train(self, epochs: int, save_path, val_every: int = 1, accumulation_step: int = 1):
        ...
        for epoch in range(epochs):
            ...
            for it, inputs in tqdm(enumerate(self.train_dataloader), total=len(self.train_dataloader), desc='Training'):
                ...
                if not isinstance(self.criterion, DiceLoss):
                    ...
                    loss = self.criterion(logits.view(-1, num_targets), labels.flatten(),
                                          mask=(labels.flatten() >= 0)) / accumulation_step
                loss.backward()
